...
final class FLBuilder {
	...
	static public function enqueue_layout_styles_scripts($post_id)
	{
		if(FLBuilderModel::is_builder_enabled()) {
			...
			$all_js[$j] = $asset_info['js_url'];
			$j++;
			update_post_meta($post_id,'rendered_css',$asset_info['css']);
			update_post_meta($post_id,'cu_all_css',$all_css);
			update_post_meta($post_id,'cu_all_js',$all_js);
		}
	}
	...
	static public function render_content( $content )
	{
		...
		if( $enabled && ! $rendering && ( $in_loop || $is_global ) ) {
			...
			/*echo '<script type="text/javascript" src="'.$upload_path.'/bb-plugin/cache/'.$post_id.'-layout.js"></script>';*/
			echo '<link rel="stylesheet" id="_custom_fl-builder-layout-'.$post_id.'" href="'.$upload_path.'/bb-plugin/cache/'.$post_id.'-layout.css" />';
		...
		update_post_meta($post_id,'rendered_content',$content);
		return $content;
	}
	...
	static public function render_css()
	{
		...
		if(!empty($css)) {
			...
			update_post_meta($post_id,'rendered_css',$asset_info['css']);
			update_post_meta($post_id,'rendered_css_content',addslashes($css));
		}
	}
	...
	static public function render_js()
	{
		...
		update_post_meta($post_id,'rendered_js_content',addslashes($js));
	}
