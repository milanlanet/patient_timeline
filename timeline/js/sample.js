...
{
	var opts = jQuery.extend({}, SAMPLE_BUTTON_DEFAULTS, options),
		$buttonBar = jQuery("#"+opts.parent_id),
	...
	.click(function(e){
		...
		if($pre.is(":visible")){
			jQuery(this).text("Source code");
		}else{
			jQuery(this).text("Hide source");
		}
		$pre.toggle("slow");
		return false;
	});*/
...
//    sourceCode = $.trim(sourceCode);
	...
	if(opts.header){
		jQuery("<h5 />", {text: opts.header}).appendTo($("p.pmButtons"));
	}
	if( !jQuery(".pmButtons").length ){
		jQuery.error("addSampleButton() needs a container with id #sampleButtons");
	}
...
{
	var $source = jQuery("#sourceCode");
	jQuery("#codeExample").clickToggle(
		function(){
			$source.show("fast");
			if( !this.old ){
				this.old = jQuery(this).html();
				jQuery.get(this.href, function(code){
			...
			jQuery(this).html("Hide source code");
		},
		function(){
			jQuery(this).html(this.old);
			$source.hide("fast");
		}
	);
	if(jQuery.ui){
...
*/
		jQuery("p.sample-links").after("<p class='version-info'>" + info + "</p>");
	}
}
